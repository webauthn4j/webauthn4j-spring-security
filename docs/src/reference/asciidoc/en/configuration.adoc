...
Please apply `WebAuthnAuthenticationProviderConfigurer` to a configuration class inherits `WebSecurityConfigurerAdapter`.
...
WebAuthn4J Spring Security looks up a user through the `WebAuthnAuthenticatorService` interface.
...
In the case where it is needed to show a different login view like one asks an authenticator for the user whose first factor is verified but second factor is not provided yet.
One way is to switch the view depending on whether the current `Authentication` instance is anonymous or not.
