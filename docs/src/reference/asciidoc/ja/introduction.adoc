...
2018年12月現在、デスクトップ版Google Chrome 70以降およびデスクトップ版Mozilla Firefox 60以降でWebAuthnは利用可能です。 但し、認証デバイスのサポートはFIDO-U2Fの認証トークンに限定されており、指紋認証などによるユーザー検証機能がついた 認証デバイスはサポートしていないため、パスワードレス認証は利用できません。 Microsoft Edgeでは、Windows 10 October 2018 Update以降でWebAuthnが利用可能です。EdgeではFIDO-U2Fの認証トークンだけでなく、 Windows Helloを認証デバイスとして使用できるため、パスワードレス認証をサポートしています。
